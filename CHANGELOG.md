...
### 新增 (Added)
- 模型成本看板页：`/admin/cost-dashboard` 展示今日总费用/总 Token/活跃模型数汇总卡片 + 按模型维度的 Token 消耗明细表格（v0.50.8）

## [0.50.4] — 2026-04-04

### 新增 (Added)
- 审批看板操作组件（v0.50.3-4 合并实现）：审批中心三列看板（待分配/待审批/已完成），每列卡片显示文档信息、审批人、审批备注、状态标签，空状态友好提示，嵌入到现有 review/page.tsx 中（v0.50.3+4）
