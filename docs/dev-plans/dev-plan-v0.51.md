...
建议：`v0.51.1 → v0.51.2 → v0.51.3 → v0.51.5 → v0.51.6 → v0.51.7 → v0.51.4 → v0.51.8 → v0.51.9 → v0.51.10 → v0.51.11 → v0.51.12 → v0.51.13`
> v0.51.13 放在最后：部署指南需要 v0.51.1-2 确定最终部署架构后才能准确撰写
...
### v0.51.13: 团队文档 — 部署指南 + 开发者上手文档

**任务版本号：** v0.51.13
**优先级：** P2
**前置依赖：** v0.51.1-2（部署架构确定后才能写部署指南）

---

#### 背景与目标

**现状：** 项目缺乏面向团队成员的结构化文档。新成员上手依赖口头传递，部署流程仅存在于零散配置文件中。PRD §10 要求"团队开发自给自足"，需要系统性文档支撑。

**目标（Goal）：** 编写 3 份核心团队文档：部署指南、开发者上手指南、架构决策记录（ADR）索引，确保任何新成员可在 1 天内完成环境搭建并理解系统全貌。
...
- `docs/deployment-guide.md` — 部署指南：
  - 环境要求（Python 3.12 / Node.js / PostgreSQL / Redis / MinIO）
  - Docker Compose 本地开发环境一键启动
  - 生产环境部署清单（含 v0.51.1 S3 切换 + v0.51.2 Redis Sentinel 配置）
  - 环境变量完整列表（从 settings.py 自动导出 + 说明）
  - Alembic migration 执行步骤
  - 常见部署问题 FAQ

- `docs/developer-guide.md` — 开发者上手指南：
  - 项目结构说明（4 服务 + 4 包 + 1 前端）
  - 本地开发环境搭建步骤（clone → install → migrate → seed → run）
  - 开发工作流（分支策略、commit 规范、PR 流程）
  - 测试执行指南（pytest / vitest / 集成测试）
  - 常用命令速查表

- `docs/adr/README.md` — 架构决策记录索引：
  - ADR-001: 微服务拆分策略（4 服务 + 4 共享包）
  - ADR-002: Pipeline 9 阶段设计
  - ADR-003: PA Pass JWT 认证集成
  - ADR-004: 双列 embedding 共存策略（JSONB + pgvector）
  - ADR-005: SKILL 驱动 Pipeline 设计
  - 每条 ADR 包含：背景、决策、后果、状态

**业务规则：**
① 部署指南的环境变量列表必须与 `settings.py` 实际字段一一对应
② 开发者指南的步骤必须可在全新机器上复现（可验证性要求）
③ ADR 使用标准 [Michael Nygard 格式](https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions)
④ 所有文档使用中文撰写，技术术语保留英文
...
- [ ] 新成员按 developer-guide.md 步骤可在 1 小时内启动本地开发环境
- [ ] deployment-guide.md 环境变量列表与 settings.py 完全一致
- [ ] Docker Compose 一键启动命令可成功运行全部服务
- [ ] ADR 索引覆盖 5 个以上关键架构决策
- [ ] 所有文档无 TODO 占位符
...
**包含：** 3 份文档撰写（~500 行 Markdown）
**不包含：** CI/CD pipeline 文档（依赖实际 CI 环境确定后补充）；API 文档（由 OpenAPI 自动生成）

---

#### 预估工作量

- Phase 1 梳理当前部署流程和架构决策：2 小时
- Phase 2 撰写 3 份文档：4 小时
- Phase 3 验证（新环境测试部署指南可行性）：2 小时
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|---------|
| Docker Compose 配置过时 | 中 | 低 | Phase 1 先在本地验证 compose up |
| 环境变量列表遗漏 | 低 | 中 | 从 settings.py 程序化导出，非手写 |

> ⚠️ **Phase 1 前置确认：**
> 1. 确认 Docker Compose 文件是否存在及其内容
> 2. 确认当前有哪些 ADR 级别的架构决策需要记录
...
合计：4 张新表，3 张表修改，6 次 Alembic migration。v0.51.13 无 DB 变更（纯文档任务）。
